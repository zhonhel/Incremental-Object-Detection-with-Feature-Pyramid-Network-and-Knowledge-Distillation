...
class GenerateProposalsOp(object):
    ...
    def __init__(self, anchors, spatial_scale, train,m_slvl, reg_weights=(1.0, 1.0, 1.0, 1.0)):
    ...
    def forward(self, inputs, outputs):
        ...
        tmp1000='rpn_cls_probs_fpn'+self._slvl+'_s'
        while True:
            if(self._mc.get(tmp1000)=='yidu'):
                break             
        self._mc.replace('rpn_cls_probs_fpn'+self._slvl,inputs[0].data)
        self._mc.replace('rpn_cls_probs_fpn'+self._slvl+'_s','weidu')
        
        
        tmp1001='rpn_bbox_pred_fpn'+self._slvl+'_s'
        while True:
            if(self._mc.get(tmp1001)=='yidu'):
                break               
        self._mc.replace('rpn_bbox_pred_fpn'+self._slvl,inputs[1].data)
        self._mc.replace('rpn_bbox_pred_fpn'+self._slvl+'_s','weidu')

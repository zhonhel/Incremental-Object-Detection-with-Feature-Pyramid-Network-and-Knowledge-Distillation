...
def add_fast_rcnn_blobs(blobs, im_scales, roidb,mc):
    ...
    while True:
        if(mc.get('rois_s')=='weidu'):
            break        
    rois_oldC=mc.get('rois')
    mc.replace('rois_s','yidu')

        
    mySplit=np.zeros((2,),dtype=np.int32)
    mySplit[0]=blobs['rois'].shape[0]
    mySplit[1]=rois_oldC.shape[0]
    workspace.FeedBlob('split_tensor', mySplit)

    
    blobs['rois'] = np.concatenate((blobs['rois'],rois_oldC),axis=0)

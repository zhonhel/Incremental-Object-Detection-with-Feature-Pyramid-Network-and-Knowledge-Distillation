...
def add_fpn_rpn_outputs(model, blobs_in, dim_in, spatial_scales):
    ...
    for lvl in range(k_min, k_max + 1):
        ...
        if lvl == k_min:
            ...
            rpn_cls_logits_toothbrush = model.Conv(
                conv_rpn_fpn,
                'rpn_cls_logits_toothbrush' + slvl,
                dim_in,
                num_anchors,
                kernel=1,
                pad=0,
            ...
            rpn_bbox_pred_toothbrush = model.Conv(
                conv_rpn_fpn,
                'rpn_bbox_pred_toothbrush' + slvl,
                dim_in,
                4 * num_anchors,
                kernel=1,
                pad=0,
            ...
            rpn_cls_logits_toothbrush = model.ConvShared(
                conv_rpn_fpn,
                'rpn_cls_logits_toothbrush' + slvl,
                dim_in,
                num_anchors,
                ...
                pad=0,
                stride=1,
                weight='rpn_cls_logits_toothbrush' + sk_min + '_w',
                bias='rpn_cls_logits_toothbrush' + sk_min + '_b'
            )
            rpn_bbox_pred_toothbrush = model.ConvShared(
                conv_rpn_fpn,
                'rpn_bbox_pred_toothbrush' + slvl,
                dim_in,
                4 * num_anchors,
                ...
                pad=0,
                stride=1,
                weight='rpn_bbox_pred_toothbrush' + sk_min + '_w',
                bias='rpn_bbox_pred_toothbrush' + sk_min + '_b'
            )
        ...
        if not model.train or cfg.MODEL.FASTER_RCNN:
            ...
            rpn_cls_probs_toothbrush = model.net.Sigmoid(
                rpn_cls_logits_toothbrush, 'rpn_cls_probs_toothbrush' + slvl
            )
            model.GenerateProposals(
                [rpn_cls_probs_toothbrush, rpn_bbox_pred_toothbrush, 'im_info'],
                ['rpn_rois_toothbrush' + slvl, 'rpn_roi_probs_toothbrush' + slvl],
                anchors=lvl_anchors,
                spatial_scale=sc
            )
...
def add_fpn_rpn_losses(model):
    ...
    for lvl in range(cfg.FPN.RPN_MIN_LEVEL, cfg.FPN.RPN_MAX_LEVEL + 1):
        ...
        model.net.SpatialNarrowAs(
            ['rpn_labels_int32_wide_fpn' + slvl, 'rpn_cls_logits_toothbrush' + slvl],
            'rpn_labels_int32_toothbrush' + slvl
        )
        for key in ('targets', 'inside_weights', 'outside_weights'):
            model.net.SpatialNarrowAs(
                ...
                'rpn_bbox_' + key + '_toothbrush' + slvl
            )
        loss_rpn_cls_toothbrush = model.net.SigmoidCrossEntropyLoss(
            ['rpn_cls_logits_toothbrush' + slvl, 'rpn_labels_int32_toothbrush' + slvl],
            'loss_rpn_cls_toothbrush' + slvl,
            normalize=0,
            scale=1*2/3*(
        ...
        loss_rpn_bbox_toothbrush = model.net.SmoothL1Loss(
            [
                'rpn_bbox_pred_toothbrush' + slvl, 'rpn_bbox_targets_toothbrush' + slvl,
                'rpn_bbox_inside_weights_toothbrush' + slvl,
                'rpn_bbox_outside_weights_toothbrush' + slvl
            ],
            'loss_rpn_bbox_toothbrush' + slvl,
            beta=1. / 9.,
            scale=1*2/3*model.GetLossScale(),
        )
        
        
        model.get_weightsRPN(slvl,'cls_probs')
        loss_rpn_cls_fpn = model.net.SmoothL1Loss(
            [
                'rpn_cls_probs_fpn' + slvl, 'freeze_rpn_cls_probs_fpn' + slvl,
                'freeze_inside_weights_cls' + slvl,
                'freeze_outside_weights_cls' + slvl
            ],
            'loss_rpn_cls_fpn' + slvl,
            beta=1. / 9.,
            scale=2*2/3*model.GetLossScale(),
        )
        model.get_weightsRPN(slvl,'bbox_pred')
        loss_rpn_bbox_fpn = model.net.SmoothL1Loss(
            [
                'rpn_bbox_pred_fpn' + slvl, 'freeze_rpn_bbox_pred_fpn' + slvl,
                'freeze_inside_weights_bbox' + slvl,
                'freeze_outside_weights_bbox' + slvl
            ],
        ...
        loss_gradients.update(
            blob_utils.
            get_loss_gradients(model, [loss_rpn_cls_toothbrush, loss_rpn_bbox_toothbrush,loss_rpn_cls_fpn,loss_rpn_bbox_fpn])
        )
        model.AddLosses(['loss_rpn_cls_toothbrush' + slvl, 'loss_rpn_bbox_toothbrush' + slvl,'loss_rpn_cls_fpn' + slvl,'loss_rpn_bbox_fpn' + slvl])

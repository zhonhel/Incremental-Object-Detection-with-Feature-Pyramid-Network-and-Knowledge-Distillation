...
from detectron.ops.distillLossOp import distillLossOp
from detectron.ops.get_weightsOpRPN import get_weightsOpRPN
from detectron.ops.get_weightsOpFastrcnn import get_weightsOpFastrcnn
...
class DetectionModelHelper(cnn.CNNModelHelper):
    ...
    def CollectAndDistributeFpnRpnProposals(self):
        ...
        # Prepare input blobs
        rois_names = ['rpn_rois_toothbrush' + str(l) for l in range(k_min, k_max + 1)]
        score_names = [
            'rpn_roi_probs_toothbrush' + str(l) for l in range(k_min, k_max + 1)
        ]
        ...
        name = 'CollectAndDistributeFpnRpnProposalsOp'
    ...
    def get_weightsRPN(self,slvl,tag):
        
        blobs_in=[]
        if tag=='cls_probs':
            blobs_out=['freeze_rpn_cls_probs_fpn'+slvl,'freeze_inside_weights_cls'+slvl,'freeze_outside_weights_cls'+slvl]
        if tag=='bbox_pred':
            blobs_out=['freeze_rpn_bbox_pred_fpn'+slvl,'freeze_inside_weights_bbox'+slvl,'freeze_outside_weights_bbox'+slvl]                
                     
        blobs_out = [core.ScopedBlobReference(b) for b in blobs_out]
        self.net.Python(get_weightsOpRPN(self.train,slvl,tag).forward)(blobs_in, blobs_out, name='get_weightsOpRPN'+slvl+tag)
        
    def get_weightsFastrcnn(self):
        
        blobs_in=['cls_score_toothbrush']

        blobs_out=['freeze_cls_score','fastrcnn_cls_loss_inside_weights','fastrcnn_cls_loss_outside_weights',
                  'freeze_bbox_pred','fastrcnn_reg_loss_inside_weights','fastrcnn_reg_loss_outside_weights']
              
        blobs_in = [core.ScopedBlobReference(b) for b in blobs_in]       
        blobs_out = [core.ScopedBlobReference(b) for b in blobs_out]
        self.net.Python(get_weightsOpFastrcnn(self.train).forward)(blobs_in, blobs_out, name='get_weightsOpFastrcnn')
    ...
    def distillLoss(self,blobs_in,blobs_out,temperature):
        ...
        self.net.Python(distillLossOp(self.train,temperature).forward,
                        distillLossOp(self.train,temperature).backwardd)(blobs_in, blobs_out, name='distillLoss') 
